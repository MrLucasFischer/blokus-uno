...
(defun iniciar (caminho)
  ...
  (let* (
    ...
    (cond
     ((equal tipo-de-jogo 'humano) (comecar-jogo-humano no tempo-limite profundidade-escolhida jogador-escolhido caminho))
...
(defun comecar-jogo-humano (no tempo-limite profundidade-maxima peca-humano caminho &optional (humano-passou-vez nil) (maquina-passou-vez nil))
  ...
  (cond
   ...
   ((and (null (get-pai-no no)) (= (get-valor-jogador-no no) 2))
     ...
     ((= (get-valor-jogador-no no) peca-humano)
      (let* 
        ...
        (cond
         ...
         ((zerop (+ (length jogadas-possiveis-pequena) (length jogadas-possiveis-media) (length jogadas-possiveis-cruz))) (format T "~%   ---Nao tem jogadas possiveis para este tabuleiro--~%") (comecar-jogo-humano (cria-no (get-estado-no no) (get-pecas-jogador1-no no) (get-pecas-jogador2-no no) (trocar-jogador no) (get-profundidade-no no) 'MAX (get-pai-no no)) tempo-limite profundidade-maxima peca-humano caminho T maquina-passou-vez))

         (T
          (let* 
              (
               (apresentar-informacao (mostrar-info caminho tabuleiro pecas-jogador pecas-maquina jogadas-possiveis-pequena jogadas-possiveis-media jogadas-possiveis-cruz))
            ...
            (comecar-jogo-humano novo-no tempo-limite profundidade-maxima peca-humano caminho nil maquina-passou-vez) ;Dizer que o jogador nao passou a vez
          )
         )
        )
      )
      ...
      (let* 
        ...
        (let 
         ...
         ;Caso a maquina tenha passado a vez
           ...
           (T (comecar-jogo-humano novo-no tempo-limite profundidade-maxima peca-humano caminho humano-passou-vez nil)) ;Dizer que a maquina nao passou a vez
...
(defun ler-tabuleiro (caminho)
  ...
  (progn
    ...
    (format T "~% -> 1 - Tabuleiro vazio 14x14")
    (format T "~% -> 2 - Tabuleiro vazio 10x11")
    (format T "~% -> 3 - Tabuleiro vazio 20x10")
    (format T "~% -> 4 - Tabuleiro a)")
    (format T "~% -> 5 - Tabuleiro b)")
    (format T "~% -> 6 - Tabuleiro c)")
    (format T "~% -> 7 - Tabuleiro d)")
    (format T "~% -> 8 - Tabuleiro e)")
    (format T "~% -> 9 - Tabuleiro f)~%")
    ...
    (let (
      ...
      (with-open-file (ficheiro (concatenate 'string caminho "/problemas.dat") :direction :input)
        (let* (
               (tabuleiro-catorze (ler-tabuleiro-ficheiro ficheiro))
               (tabuleiro-dez (ler-tabuleiro-ficheiro ficheiro))
               (tabuleiro-vinte (ler-tabuleiro-ficheiro ficheiro))
        ...
        (cond
         ...
         ((= resposta 1) tabuleiro-catorze)
         ((= resposta 2) tabuleiro-dez)
         ((= resposta 3) tabuleiro-vinte)
         ((= resposta 4) tabuleiro-a)
         ((= resposta 5) tabuleiro-b)
         ((= resposta 6) tabuleiro-c)
         ((= resposta 7) tabuleiro-d)
         ((= resposta 8) tabuleiro-e)
         ((= resposta 9) tabuleiro-f)
         ;((= resposta 10) tabuleiro-g)
...
;; mostrar-info
(defun mostrar-info (caminho tabuleiro pecas-jogador pecas-maquina jogadas-pequena jogadas-media jogadas-cruz)
...
;; escrever-log-ficheiro-humano
(defun escrever-log-ficheiro-humano (caminho tabuleiro pecas-jogador pecas-maquina jogadas-pequena jogadas-media jogadas-cruz)
  "Funcao que ira escrever num ficheiro os dados obtidos de um jogada de um humano"
  (with-open-file (ficheiro-log 
                   (concatenate 'string caminho "/log.dat")
                   :direction :output
                   :if-exists :append
    ...
    (progn
      (formatar-tabuleiro tabuleiro ficheiro-log)
      (format ficheiro-log "~%~%  -Suas pecas: ~A  | Pecas do Adversario: ~A" pecas-jogador pecas-maquina)
      (format ficheiro-log "~%~%  -Escolha a sua jogada:")
      (cond
       ((not (null jogadas-pequena)) (format ficheiro-log "~% Jogadas possiveis para a peca pequena: ~% ~A" jogadas-pequena))
       (t nil)
       )
      (cond
       ((not (null jogadas-media)) (format ficheiro-log "~%~% Jogadas possiveis para a peca media: ~% ~A" jogadas-media))
       (t nil)
       )
      (cond
       ((not (null jogadas-cruz)) (format ficheiro-log "~%~% Jogadas possiveis para a peca cruz: ~% ~A~%" jogadas-cruz))
       (t nil)
       )
      (format ficheiro-log "~%~%---------------------------------------------------------------------------------------------~%")
    )
